...
키움증권 자동 매매 프로그램/
│
├── 📄 키움증권 자동매매.py     # 메인 UI 및 트레이딩 로직 (2,400+줄)
├── 📄 config.py                # 설정 및 상수 정의 (430+줄)
├── 📄 strategy_manager.py      # 매매 전략 로직 (1,062줄)
├── 📄 sound_notifier.py        # 사운드 알림
├── 📄 profile_manager.py       # 다중 프로필 관리
├── 📄 telegram_notifier.py     # 텔레그램 알림
├── 📄 ui_dialogs.py            # UI 다이얼로그 모음 (한글화 완료)
...
| 파일 | 라인 수 | 주요 기능 |
|------|---------|-----------|
| `키움증권 자동매매.py` | 2,400+ | 메인 UI, 시스템 트레이, 자동실행, API 연결 |
| `strategy_manager.py` | 1,062 | 매매 전략 로직, 지표 계산 |
| `config.py` | 450+ | 설정 상수, 시스템 설정 기본값 |
| `ui_dialogs.py` | 513 | 검색/주문/설정 다이얼로그 (한글화) |
| `dark_theme.py` | 500+ | 모던 다크 테마 QSS |
...
빌드된 파일은 `dist/KiwoomTrader_v4.5/` 폴더에 생성됩니다.

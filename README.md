...
- **분할 매수 설정 UI** - 저장/프로필/UI까지 연결되어 있으며 주문 분할 라우팅은 후속 구현 범위
...
- `IMPLEMENTATION_REVIEW_2026-03-25.md`를 추가해 코드 기준 잠재 이슈와 후속 구현 과제를 별도로 정리했습니다.
- `분할 매수`는 현재 UI/설정/프로필 경로까지 연결되어 있으며, 실주문 분할 제출은 아직 후속 구현 범위입니다.
- 전략팩에서 SHORT 방향을 반환하는 전략(`pairs_trading_cointegration`, `stat_arb_residual`, `ff5_factor_ls`)은 현재 백테스트/시뮬레이션 범위로 이해하는 편이 맞습니다.

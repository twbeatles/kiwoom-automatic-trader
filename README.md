...
- 🧠 **시장 인텔리전스** - 뉴스/공시/검색트렌드/매크로를 규칙 기반으로 해석해 진입 가드에 반영
...
- **분할 매수** - `지정가 우선` 정책에서 분할 가격으로 즉시 다건 지정가 주문 제출
...
- **수동 주문** - 실계좌 주문마다 보호 확인 + 코드/지정가 검증 + 예약금 즉시 반영
- **예약 매매** - 시작 실패 시 다음 타이머 tick에서 재시도하는 안전한 시작/종료 예약
- **다중 프로필** - 설정 프로필 저장/불러오기
...
| `app/main_window.py` | 736 | 메인 클래스 선언 + 공용 시그널 + 런타임 상태 |
...
이번 업데이트는 코드 분할 리팩토링 이후 문서와 빌드 메타데이터를 현재 구조에 맞춰 다시 맞춘 정합성 동기화다.

### 반영된 핵심 변경

- `strategy_manager.py` 를 orchestration 레이어로 축소하고 실제 책임을 `strategies/manager_mixins/` 로 분리
- `ui_dialogs.py` 를 호환 re-export 레이어로 유지하고 실제 다이얼로그 구현을 `dialogs/` 패키지로 이동
- `KiwoomTrader.spec` 에 `dialogs` 패키지 수집 경로를 반영
- `.gitignore` 에 PyInstaller 보조 작업 디렉터리/백업 spec 패턴을 추가

### 현재 구조에서 기억할 점

- 외부 import는 계속 `from strategy_manager import StrategyManager`, `from ui_dialogs import ...` 를 그대로 사용한다.
- 실제 구현 탐색은 `strategy_manager.py` 대신 `strategies/manager_mixins/`, `ui_dialogs.py` 대신 `dialogs/` 를 먼저 보는 편이 정확하다.
...
- 예약 매매 시작 실패 시 `schedule_started` 가 고착되지 않도록 시작 상태머신을 교정
...
| 기능 | UI 노출 | 설정 저장 | 자동매매 연결 | 백테스트/연구 | 테스트 커버 |
|------|---------|-----------|---------------|----------------|-------------|
| 예약 매매 | O | O | O | - | O |
| 수동 주문 | O | - | O | - | O |
...
```bash
python -m pytest -q tests/unit
python -m compileall -q app api data backtest strategies portfolio
```

- 결과: **`tests/unit` 전체 113개 테스트 통과**
...
- 기존 provider 확장:
  - `data/providers/dart_provider.py` -> 최근 공시 조회 + corp code 캐시
  - `data/providers/macro_provider.py` -> FRED 다중 시리즈 번들 조회
- 전략/전략팩/백테스트에 다음 guard/filter를 통합:
  - `news_risk_guard`
  - `disclosure_event_guard`
  - `macro_regime_guard`
  - `theme_heat_filter`
  - `intel_fresh_guard`
- 백테스트에 `BacktestIntelligenceEvent` sidecar replay를 추가해 라이브와 동일한 시장 인텔리전스 차단 의미를 재생
...
## 🔄 2026-03-09 정적 분석 및 문서 동기화 업데이트

이번 업데이트는 repo 전반의 정적 분석 기준, 패키징 메타데이터, 문서를 현재 코드 기준으로 다시 맞춘 동기화입니다.
...
- `README.md` / `CLAUDE.md` / `GEMINI.md` / 구조 분석 문서를 현재 파일 구조와 검증 절차 기준으로 동기화
...
- 결과: `pyright .` -> **0 errors, 0 warnings** (2026-03-09 당시 환경)
- 결과: `python -m pytest -q tests/unit` -> **83 passed** (2026-03-09)
- 인코딩 점검: UTF-8 디코드 실패 및 `U+FFFD` 없음
...
- `data/providers/` - `kiwoom`, `dart`, `macro`, `csv`, `news`, `naver_trend`, `ai` 데이터 프로바이더 계층
...
이번 업데이트는 2026-03-07 기능 리뷰 제안사항을 코드/테스트 기준으로 반영한 동기화 내용입니다.

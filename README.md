...
- **🔍 조건 검색**: 키움 HTS에서 작성한 사용자 조건검색식을 WebSocket `CNSRLST`/`CNSRREQ`(ka10171/ka10172)로 조회해 포착 종목을 감시
- **🏆 순위**: 당일 거래량 상위(`ka10030`), 등락률 상위(`ka10027`) 공식 REST 랭킹 조회
...
트레이딩 중 신속한 조작을 위해 14가지 글로벌 단축키를 지원합니다.
...
| **`Ctrl + =`** | **UI 확대** | 글자·여백 한 단계 확대 (고배율 디스플레이용) |
| **`Ctrl + -`** | **UI 축소** | 글자·여백 한 단계 축소 |
...
│   └── _rest_transport/helpers/market/account/orders/discovery  # REST SRP 분할 + rest_client facade
├── app/
...
- `backtest/engine.py` → `backtest/models/_base/_metrics/_policy/_guards/_intel_events` + `EventDrivenBacktestEngine` facade.
- `app/mixins/dialogs_profiles.py` → `app/features/dialogs/` 4종 믹스인 + composite facade (기존 테스트 patch seam 보존).
- 구조 변경 이력은 `CLAUDE.md`의 날짜별 동기화 메모에서 확인합니다.

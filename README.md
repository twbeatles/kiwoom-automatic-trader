...
- **수동 주문** - 실계좌 주문마다 보호 확인 + 코드/지정가/매도 가능수량 검증 + 예약금 즉시 반영
...
## 🔄 2026-04-12 API 모드·외부보유·종료 정합성 업데이트

이번 업데이트는 실제 코드가 이미 갖고 있는 `모의/실전 API 라우팅`, `유니버스 외 보유 종목 추적`, `중지 시 주문 정리` 동작을 문서와 빌드 설정에 맞춘 정합성 보강입니다.

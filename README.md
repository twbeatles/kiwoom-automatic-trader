...
| **키보드 단축키** | 11가지 | 빠른 조작 |
| **레이아웃 조절** | 드래그 | 주식 테이블/로그 패널 크기 조절 |

### 🔧 UI/UX 개선 (추가 업데이트)
- **레이아웃 개선** - 스플리터 적용으로 테이블/로그 영역 크기 조절 기능 추가
- **입력 안전장치** - 마우스 휠 스크롤에 의한 실수 방지 (포커스 시에만 작동)
- **설정 가독성** - 고급 설정 탭 스크롤 영역 적용
- **API 검증** - 연결 시 입력값 검증 및 자동 탭 전환 기능 강화

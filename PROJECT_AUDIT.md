...
본 감사는 최근 진행된 키움 Open API 규격 정상화(헤더 `api-id`, 주문 엔드포인트 단일화 `/api/dostk/ordr`, 확장 TR `kt00001`/`ka10076`/`ka10079`/`ka10054` 연동) 및 자동매매 고도화 기능(Chandelier Exit, 호가 스프레드 가드, 세션 매매 성과 분석 등)을 포함하여 시스템 전반의 기능 구현 상태를 점검했습니다. 2026-09-14 기준으로 순위(`ka10030`/`ka10027`), 기관/프로그램(`ka10045`/`ka90013`), 업종현재가(`ka20001`), 조건검색(WebSocket `CNSRLST`/`CNSRREQ`)도 공식 예제 경로로 정렬했습니다.

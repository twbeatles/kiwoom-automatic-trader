...
- UI `API 설정` 탭에서 `App Key`, `Secret Key` 입력
- `API 연결` 버튼 클릭
- 계좌 목록 수신
- 계좌 선택
...
8. 이 단계까지는 AI를 끈다.
9. `🧠 인텔리전스` 탭에서 `source health`, `action policy`, `size multiplier`, `exit policy` 를 확인한다.
10. `📼 리플레이` 탭에서 이벤트 로그와 감사 로그가 누적되는지 확인한다.
...
- `📼 리플레이` 탭에서 최근 이벤트 확인
- `🩺 진단` 탭에서 `sync_failed`, `external error`, `source health` 확인
...
5. 매일 장 종료 후 `📼 리플레이` 탭과 `decision_audit.jsonl` 을 검토한다.
